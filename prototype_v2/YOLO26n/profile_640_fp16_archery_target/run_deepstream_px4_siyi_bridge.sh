...
TILT_DEADZONE="${TILT_DEADZONE:-0.10}"
TILT_SMOOTH_ALPHA="${TILT_SMOOTH_ALPHA:-0.18}"
TILT_FAST_SMOOTH_ALPHA="${TILT_FAST_SMOOTH_ALPHA:-0.45}"
TILT_FAST_ERROR_ZONE="${TILT_FAST_ERROR_ZONE:-0.35}"
...
TILT_FEEDFORWARD_GAIN="${TILT_FEEDFORWARD_GAIN:-0.04}"
...
MAX_YAW_RATE_DPS="${MAX_YAW_RATE_DPS:-90.0}"
MAX_PITCH_RATE_DPS="${MAX_PITCH_RATE_DPS:-90.0}"
...
MIN_YAW_ANGLE_DEG="${MIN_YAW_ANGLE_DEG:--100.0}"
MAX_YAW_ANGLE_DEG="${MAX_YAW_ANGLE_DEG:-100.0}"
MIN_PITCH_ANGLE_DEG="${MIN_PITCH_ANGLE_DEG:--90.0}"
MAX_PITCH_ANGLE_DEG="${MAX_PITCH_ANGLE_DEG:-25.0}"
TARGET_MEMORY_ENABLE="${TARGET_MEMORY_ENABLE:-1}"
TARGET_MEMORY_LEVEL="${TARGET_MEMORY_LEVEL:-1}"
SEARCH_PITCH_DEFAULT="${SEARCH_PITCH_DEFAULT:--45.0}"
CANDIDATE_STABLE_FRAMES="${CANDIDATE_STABLE_FRAMES:-3}"
SHORT_LOST_TIMEOUT_MS="${SHORT_LOST_TIMEOUT_MS:-450}"
...
SEARCH_RATE_DEG_S="${SEARCH_RATE_DEG_S:-18.0}"
UNCERTAINTY_GROWTH_RATE="${UNCERTAINTY_GROWTH_RATE:-0.35}"
CONFIDENCE_THRESHOLD="${CONFIDENCE_THRESHOLD:-0.35}"
EDGE_MARGIN_THRESHOLD="${EDGE_MARGIN_THRESHOLD:-0.82}"
TRACKING_UNSTABLE_THRESHOLD="${TRACKING_UNSTABLE_THRESHOLD:-0.70}"
YAW_LOCK="${YAW_LOCK:-0}"
...
if [[ "${RUN_ARTIFACTS_ENABLE}" == "1" ]]; then
...

    {
        ...
        echo "  tilt_deadzone: ${TILT_DEADZONE}"
        echo "  tilt_smooth_alpha: ${TILT_SMOOTH_ALPHA}"
        echo "  tilt_fast_smooth_alpha: ${TILT_FAST_SMOOTH_ALPHA}"
        echo "  tilt_fast_error_zone: ${TILT_FAST_ERROR_ZONE}"
        ...
        echo "  min_yaw_angle_deg: ${MIN_YAW_ANGLE_DEG}"
        echo "  max_yaw_angle_deg: ${MAX_YAW_ANGLE_DEG}"
        echo "  min_pitch_angle_deg: ${MIN_PITCH_ANGLE_DEG}"
        echo "  max_pitch_angle_deg: ${MAX_PITCH_ANGLE_DEG}"
        echo "  yaw_lock: ${YAW_LOCK}"
        echo "  pitch_lock: ${PITCH_LOCK}"
        echo "target_memory:"
        echo "  enable: ${TARGET_MEMORY_ENABLE}"
        echo "  level: ${TARGET_MEMORY_LEVEL}"
        echo "  search_pitch_default: ${SEARCH_PITCH_DEFAULT}"
        echo "  candidate_stable_frames: ${CANDIDATE_STABLE_FRAMES}"
        echo "  short_lost_timeout_ms: ${SHORT_LOST_TIMEOUT_MS}"
        ...
        echo "  search_rate_deg_s: ${SEARCH_RATE_DEG_S}"
        echo "  uncertainty_growth_rate: ${UNCERTAINTY_GROWTH_RATE}"
        echo "  confidence_threshold: ${CONFIDENCE_THRESHOLD}"
        echo "  edge_margin_threshold: ${EDGE_MARGIN_THRESHOLD}"
        echo "  tracking_unstable_threshold: ${TRACKING_UNSTABLE_THRESHOLD}"
        echo "mavlink:"
...
CMD=(
    ...
    --min-yaw-angle-deg "${MIN_YAW_ANGLE_DEG}"
    --max-yaw-angle-deg "${MAX_YAW_ANGLE_DEG}"
    --min-pitch-angle-deg "${MIN_PITCH_ANGLE_DEG}"
    --max-pitch-angle-deg "${MAX_PITCH_ANGLE_DEG}"
    --target-memory-level "${TARGET_MEMORY_LEVEL}"
    --search-pitch-default "${SEARCH_PITCH_DEFAULT}"
    --candidate-stable-frames "${CANDIDATE_STABLE_FRAMES}"
    --short-lost-timeout-ms "${SHORT_LOST_TIMEOUT_MS}"
    ...
    --search-rate-deg-s "${SEARCH_RATE_DEG_S}"
    --uncertainty-growth-rate "${UNCERTAINTY_GROWTH_RATE}"
    --confidence-threshold "${CONFIDENCE_THRESHOLD}"
    --edge-margin-threshold "${EDGE_MARGIN_THRESHOLD}"
    --tracking-unstable-threshold "${TRACKING_UNSTABLE_THRESHOLD}"
...
if [[ "${TARGET_MEMORY_ENABLE}" == "1" ]]; then
    CMD+=(--target-memory-enable)
fi
...
export TILT_DEADZONE
export TILT_SMOOTH_ALPHA
export TILT_FAST_SMOOTH_ALPHA
export TILT_FAST_ERROR_ZONE
