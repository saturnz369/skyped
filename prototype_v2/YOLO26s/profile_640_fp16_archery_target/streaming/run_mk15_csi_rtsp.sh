#!/usr/bin/env bash
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROFILE_DIR="$(cd "${SCRIPT_DIR}/.." && pwd)"
source "${PROFILE_DIR}/skyped_profile_runtime.sh"

REPO_BIN_PATH="${SCRIPT_DIR}/csi_h264_rtsp_server"
BIN_PATH="${CSI_RTSP_SERVER_BIN:-$(skyped_profile_binary_path "csi_h264_rtsp_server" "${REPO_BIN_PATH}")}"

export SENSOR_ID="${SENSOR_ID:-0}"
export CAMERA_WIDTH="${CAMERA_WIDTH:-2028}"
export CAMERA_HEIGHT="${CAMERA_HEIGHT:-1112}"
export CAMERA_FPS_N="${CAMERA_FPS_N:-60}"
export CAMERA_FPS_D="${CAMERA_FPS_D:-1}"
export VIDEO_SOURCE="${VIDEO_SOURCE:-csi}"
export RTSP_HOST_IP="${RTSP_HOST_IP:-$(skyped_detect_rtsp_host_ip)}"
export RTSP_PORT="${RTSP_PORT:-8554}"
export RTSP_MOUNT="${RTSP_MOUNT:-/stream}"
export RTSP_ALIAS_MOUNT="${RTSP_ALIAS_MOUNT:-/main.264}"
export BITRATE="${BITRATE:-8000000}"
export IFRAME_INTERVAL="${IFRAME_INTERVAL:-30}"

if [[ ! -x "${BIN_PATH}" ]]; then
  if [[ "${BIN_PATH}" != "${REPO_BIN_PATH}" ]]; then
    echo "Configured CSI RTSP server binary is not executable: ${BIN_PATH}" >&2
    exit 1
  fi
  make -C "${PROFILE_DIR}" streaming/csi_h264_rtsp_server
fi

exec "${BIN_PATH}"
